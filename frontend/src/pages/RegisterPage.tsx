...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await api.post("/api/auth/register", { username, email, password }, { withCredentials: true });
            navigate("/login");
        } catch (err: any) {
    ...
    return (
        <div style={{ maxWidth: 480, margin: "80px auto", padding: 20, background: "white", borderRadius: 8 }}>
            <h2 style={{ textAlign: "center", marginBottom: 18 }}>Đăng ký</h2>
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                <input type="text" placeholder="Tên hiển thị" value={username} onChange={e => setUsername(e.target.value)} required style={{ padding: 10, borderRadius: 6, border: "1px solid #e5e7eb" }} />
                ...
                <input type="password" placeholder="Nhập lại mật khẩu" value={confirm} onChange={e => setConfirm(e.target.value)} required style={{ padding: 10, borderRadius: 6, border: "1px solid #e5e7eb" }} />

...
export default function Home() {
  ...
  // group + sort theo order trước khi render
  ...
  const onDragEnd = async (result: DropResult) => {
    ...
    const reSrc =
      sourceStatus === destStatus
        ? []
        : src.map((t, i) => ({ ...t, status: sourceStatus, order: i + 1 }));
    ...
    const newAll =
      sourceStatus === destStatus ? [...untouched, ...reDst] : [...untouched, ...reSrc, ...reDst];

    // cập nhật UI lạc quan
    replaceAll(newAll);

    const toUpdate = sourceStatus === destStatus ? reDst : [...reSrc, ...reDst];
    const updates = toUpdate.map((t) => ({
      id: t.id,
      updates: { status: t.status, order: t.order },
    }));
                    ...
                    {grouped[status].map((task, index) => (
                      <Draggable key={task.id.toString()} draggableId={task.id.toString()} index={index}>
                        {(drProv) => (
                          <TaskCard
                            task={task}
                            innerRef={drProv.innerRef}
                            draggableProps={drProv.draggableProps}
